...
class USB_dongle():
    ...
    def __init__(self, dongle=None):
        ...
        :rtype : object
        """
        device_list = hid.enumerate(IDENT_VENDOR_NUM, IDENT_PRODUCT_NUM)
        nDongle = len(device_list);
        print("There are ",nDongle," dongles connected.")
        for iusb in device_list:
        ...
        for d in hid.enumerate():
            if ("vendor_id" in d) and (d["vendor_id"] == IDENT_VENDOR_NUM) and ("product_id" in d) and (d["product_id"] == IDENT_PRODUCT_NUM) and device_list[tDongle-1]["path"] == d[
"path"]:
                print("USB-I2C dongle connected on port %s", d["path"])
                found = True
    ...
    def i2c_write(self,addr=1,data=[]):
        ...
        return self.__usb_command(2,len(payload),0,payload)
